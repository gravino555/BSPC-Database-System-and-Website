...
if (isset($_REQUEST['first_name'])){
	...
	$therapist_id = stripslashes($_REQUEST['therapist_id_to_update']);
	if($therapist_id==""){
	...
	$queryUpdate = "UPDATE therapist SET first_name = '" . $first_name . "', last_name = '" . $last_name . "', status = " . $status . ", experience = " . $experience . " WHERE the_id = " . $therapist_id;
	...
	if($numRowsAff==1){
	   ...
	   $query = "SELECT * FROM therapist WHERE the_id=" . $therapist_id;
...
<h2>Update Therapist Information (id=<?=$therapist_id?>)</h2> <br/>


<h5><a href="receptionist.php">Back to Control Panel</a></h5>

<div class="<?=$formClass?>">
<form action="update_therapist2.php" method="post">
	<input type="hidden" name="therapist_id_to_update" value="<?=$therapist_id?>" />
