...
<?php		
	...
	$query = "SELECT pre_id FROM `patient` WHERE pid = '$patient_id'";
	...
	if($get_pre[0] == 1){
		$query = "SELECT first_name, last_name, doc_id FROM `doctor` WHERE (SELECT pre_id FROM `patient` WHERE pid = '$patient_id') = doc_id";  
		$result = mysqli_query($con,$query);
	...
	elseif($get_pre[0] == 2){
		$query = "SELECT first_name, last_name, the_id FROM `therapist` WHERE (SELECT pre_id FROM `patient` WHERE pid = 'patient_id') = the_id";  
		$result = mysqli_query($con,$query);
...
/* get all events for the given month */
	...
	if($duplicate_flag != 1 && isset($_POST['submit'])){
		$query = "INSERT into `time_appointment` (pid, duration, date_treat, start) VALUES ('$patient_id', '1', '$date_sql', '$time_sql')";
		$result = mysqli_query($con,$query);
		...
		$query = "INSERT into `schedule` (ta_id, emp_id, pid) VALUES ('$appoint', '$test4', '$patient_id')";
		$result = mysqli_query($con,$query);
	}	
...
<form action="appoint_rec.php" method="post">
	
	<input type="text" name="date_sql" placeholder="Date" required />
	<input type="text" name="time_sql" placeholder="Time" required />
